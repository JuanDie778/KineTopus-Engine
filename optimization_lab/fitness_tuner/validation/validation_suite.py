...
# Silenciar logs internos para no saturar la terminal durante multiprocesamiento
...
def process_asset(ticker, grid_k, weights_syn, weights_real):
    ...
    try:
        ...
        results_is = []
        # Predicciones crudas guardadas temporalmente para no repetir SINDy en OOS
        predictions_map = {} 
        ...
        for k_drift in grid_k:
            try:
                ...
                # Almacenamos características In-Sample y Score Clásico
        ...
        # Estandarizar localmente para aplicar los pesos L2 aprendidos
...
def main():
    logger.info("Iniciando Suite de Validación de Auto-Tuners...")
    ...
    results = Parallel(n_jobs=-1, verbose=10)(
        delayed(process_asset)(t, grid_k, weights_syn, weights_real) for t in tickers
    )
    ...
    summary_path = os.path.join(os.path.dirname(__file__), '..', 'results', 'real', 'validation_results.csv')
    df_summary = pd.DataFrame(all_summaries)
    df_summary.to_csv(summary_path, index=False)
    logger.info(f"Guardado: {summary_path} con {len(df_summary)} filas.")
    
    # Exportar Curvas JSON
    curves_path = os.path.join(os.path.dirname(__file__), '..', 'results', 'real', 'validation_curves.json')
