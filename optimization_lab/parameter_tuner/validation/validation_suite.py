...
def load_best_parameters(mode: str, test_mode: bool) -> dict:
    ...
    # Resolviendo ruta correcta absoluta para no fallar
...
def process_asset(ticker, params_classic, params_syn, params_real):
    """Procesa un único activo: Grid Search In-Sample + Evaluación OOS de ganadores."""
    try:
        ...
        train_len = 1850
        df_train = df_clean.iloc[:train_len]
        df_val = df_clean.iloc[train_len:]
        ...
        asset_summary = []
        asset_curves = {'ticker': ticker, 'models': {}}
        ...
        try:
            ...
            if len(p_pred_bench) == len(p_val_raw) and len(p_pred_bench) > 0:
                ...
                asset_curves['models']['Benchmark (Dynamic Classic)'] = {
                    'hit_acumulado_curve': curve_hit,
                    'equity_curve': curve_eq
                }
        except Exception as e:
            logger.warning(f"Error evaluando Benchmark (Classic Dynamic) en {ticker}: {e}")
        ...
        for model_name, p in models_configs.items():
            try:
                ...
                asset_curves['models'][model_name] = {
                    'hit_acumulado_curve': curve_hit,
                    'equity_curve': curve_eq
                }
            except Exception:
        ...
        return asset_summary, asset_curves
...
def main():
    parser = argparse.ArgumentParser(description="Suite de Validación de Parámetros en Datos Reales")
    parser.add_argument('--test', action='store_true', help="Ejecutar prueba rápida de 2 activos.")
    args = parser.parse_args()
    
    logger.info("==================================================")
    logger.info(f"Iniciando Suite de Validación de Parámetros en Datos Reales {'(TEST MODE)' if args.test else ''}...")
    logger.info("==================================================")
    ...
    # 50 Activos altamente técnicos y líquidos
    all_tickers = [
    ...
    tickers = all_tickers[:2] if args.test else all_tickers
    
    logger.info(f"Procesando {len(tickers)} activos en paralelo...")
    results = Parallel(n_jobs=-1, verbose=10)(
        delayed(process_asset)(t, params_classic, params_syn, params_real) for t in tickers
    )
    ...
    output_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'results', 'real'))
    ...
    logger.info("¡Validación en Datos Reales finalizada con éxito!")
